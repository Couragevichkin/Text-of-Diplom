...
\usepackage{rotating} %поворот таблиц (вырезано)

\usepackage{pdfpages} % для вставки pdf страниц в документ
...
\begin{document}
\includepdf[]{Title} % Титульный лист с подписью
\includepdf[]{Tasks for Diplom} % Задание на диплом
\input{DiplomProject-Annotation.tex} % Аннотация
\input{DiplomProject-Abstract.tex} % Абстракт
\tableofcontents %оглавление, которое генерируется автоматически
\input{DiplomProject-Input}% введение
\input{DiplomProject-PrincipleOfWork.tex} % описание структуры устройства
\input{DiplomProject-DescriptionOfWorkPower.tex} % описание принципа работы системы питания
\input{DiplomProject-DescriptionOfWorkMeas.tex} % описание принципа работы измерительной части
\input{DiplomProject-SubsystemEthernet.tex} % описание принципа работы подсистемы Ethernet
\input{DiplomProject-PO.tex} % Описание ПО
\input{DiplomProject-FINAL.tex} % Заключение
\input{DiplomProject-Bibliographity} % ссылки на литературу
\input{Additional A.tex} % приложение А
\end{document}